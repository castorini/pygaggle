...
Keeping computational resources in mind, our instructions primarily focus on a 105 query subset of MS MARCO dev set. 
...
*Note: Run the following instructions at root of this repo. Installation must have be done from source.*

## Models

+ monoBERT-Large: Passage Re-ranking with BERT [(Rodrigo et al., 2019)](https://arxiv.org/pdf/1901.04085.pdf)
...
We're first going to download the queries, qrels and run files corresponding to the MS MARCO subset considered. The run file is generated by following the BM25 ranking instructions. We'll store the files in the `data` directory.
...
Let's download and extract the pre-built MS MARCO index:
...
In this case, perhaps manually assigning a batch size (using option `--batch-size`) which is smaller than the default of 96, should help!

Upon completion, the re-ranked run file `run.monobert.ans_small.dev.tsv` will be available in the `runs` directory.
The following output will also be visible:
...
Great, we can also verify that the MRR@10 is indeed right using the official MS MARCO evaluation script:
